    ...
    root: Ember.Route.extend({
        index: Ember.Route.extend({
            ...
            connectOutlets: function(router) {
                router.get('applicationController').connectOutlet('notes', 'notes');
                router.get('applicationController').connectOutlet('selectedNote', 'selectedNote');
                router.get('selectedNoteController').connectControllers('notes');
            }
        })
    })
});
    ...
    createNewNote: function() {
        ...
        if (unique) {
            this.get('content').pushObject(
                Ember.Object.create({"name": this.get('newNoteName'), "value": ""})
            );
...
Ember.TEMPLATES['applicationTemplate'] = Ember.Handlebars.compile('' +
    ...
    '{{view Notes.ConfirmDialogView elementId="confirmDeleteConfirmDialog" okAction="doConfirmDelete" cancelAction="doCancelDelete" target="Notes.router" header="Delete selected note?" message="Are you sure you want to delet the selected Note? This action cannot be be undone!"}}');
...
Ember.TEMPLATES['noteListTemplate'] = Ember.Handlebars.compile('' +
    '{{#each controller}}' +
...
Ember.TEMPLATES['confirmDialog'] = Ember.Handlebars.compile(
    ...
    '<div class="modal-footer">' +
        '{{#if view.cancelAction}}' +
          '{{view Notes.BootstrapButton contentBinding="view.cancelButtonLabel" actionBinding="view.cancelAction" targetBinding="view.target"}}' +
        '{{/if}}' +
        '{{#if view.okAction}}' +
            '{{view Notes.BootstrapButton contentBinding="view.okButtonLabel" actionBinding="view.okAction" targetBinding="view.target"}}' +
        '{{/if}}' +
    '</div>'
);